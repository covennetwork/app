...
import { ConnectView, WalletConnectView } from '@/components/Connect'
...
type View = 'connect' | 'walletconnect' | 'network' | 'home' | 'pick-from' | 'pick-to' | 'review' | 'status' | 'account'
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    setView((current) => (current === 'connect' || current === 'walletconnect' ? 'home' : current))
  }, [isConnected])
  ...
  if (view === 'connect') {
    return (
      <Card viewKey="connect">
        <ConnectView connectors={connectors} onConnect={(connector) => void connect(connector)} />
      </Card>
    )
  }
  ...
  if (view === 'review' && from && to) {
    return (
      <Card viewKey="review">
        ...
        <div className="mt-10 space-y-4 text-base tracking-tight text-white/45">
          <Row>
            <span>You pay</span>
          ...
          {quote && (
            <>
              ...
              <Row>
                <span>Route</span>
                <span className="text-white">{quote.hops.map((hop) => hop.protocol).join(' → ')}</span>
              </Row>
            </>
          )}
          <Row>
      ...
      <Card viewKey={`status-${flow.title}`}>
        ...
        <div className="mt-8 space-y-4">
          ...
          {flow.hash && (
            <Note>
              <a
                className="tap underline decoration-white/25 underline-offset-4"
          ...
          {assets
            ...
            .map((asset) => (
              <Row key={asset.key}>
                ...
                <span className="text-white">{formatUnitsCompact(asset.balance ?? 0n, asset.token.decimals)}</span>
              </Row>
            ))}
        </div>
        <div className="mt-10 space-y-4">
    ...
    <Card viewKey="home">
      <div className="flex flex-wrap items-baseline justify-between gap-6">
        <button type="button" className="tap" onClick={() => setView('pick-to')}>
          <Label>
            <CountUp to={newCount} duration={1.2} separator="," className="text-white font-bold" /> new assets
      ...
      <div className="mt-6">
        <Title>
          <span className="block">
            <span className="text-white/35">Pay </span>
            <input
              ref={amountRef}
              inputMode="decimal"
              value={amount}
              ...
              className="bg-transparent text-white caret-white outline-none placeholder:text-white/20"
            />{' '}
      ...
      {/* <Nav>
        <Action tone="ghost" onClick={() => setView('pick-to')}>
          Assets
        </Action>
        <Action tone="ghost" onClick={() => setView('account')}>
